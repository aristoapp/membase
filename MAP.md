...
- **Public contract is fixed and small**: `remember`, `search`, `getContext`, `deleteOrForget` + client manifest/config generation + smoke tests. Internal terms (storage schema, graph, embedding, ranking, governance) must never leak into `packages/core`/`packages/connector-sdk` — enforced by `check-public-surface.sh`.
