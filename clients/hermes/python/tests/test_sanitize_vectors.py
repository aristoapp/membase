...
secret-assignment redaction (recall path), bearer/common-token redaction,
context-block stripping, and casual-chat detection.
"""
...
class SanitizeVectorTests(unittest.TestCase):
    ...
    def test_redact_secret_assignment_full(self) -> None:
        # SECRET_ASSIGNMENT_RE now carries the full keyword set (PRIVATE_KEY
        # included), so it must satisfy the capture-path vectors too.
        for case in self.vectors["redact_secret_assignment_full"]["cases"]:
            got = SECRET_ASSIGNMENT_RE.sub(r"\1=[REDACTED]", case["in"])
            self.assertEqual(got, case["out"], case["in"])

    def test_redact_common_tokens(self) -> None:
        for case in self.vectors["redact_common_tokens"]["cases"]:
            got = COMMON_TOKEN_RE.sub("[REDACTED_TOKEN]", case["in"])
            self.assertEqual(got, case["out"], case["in"])

    def test_redact_bearer(self) -> None:
        for case in self.vectors["redact_bearer"]["cases"]:
            got = BEARER_TOKEN_RE.sub(r"\1[REDACTED]", case["in"])
            self.assertEqual(got, case["out"], case["in"])
