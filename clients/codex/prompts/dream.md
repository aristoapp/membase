...
Read the local capture spool at `~/.membase/codex/spool/pending.jsonl`
...
second flusher finds nothing). Upload each record's `content` via the
...
is instead of deleting it. If the session started with a "pending local
