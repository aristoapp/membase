...
// OpenClaw invokes CLI actions either with a positional argument plus an
// options object, or (older gateways) with a single object carrying both.
        ...
        .action(async (queryArg?: unknown, rawOpts?: unknown) => {
          ...
          try {
            ...
            for (const [index, doc] of result.documents.entries()) {
              console.log(`${index + 1}. ${stripControlChars(doc.title)}`);
              ...
              if (doc.content) {
                console.log(`   ${stripControlChars(doc.content)}`);
              }
              console.log("");
            }
          } catch (error) {
