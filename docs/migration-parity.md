...
| Codex CLI | `clients/codex` adapter (new — no old repo to migrate), `.codex-plugin` manifest + HTTP MCP config, committed `manifests/codex/*`, install doc, workspace typecheck, generated-artifact check, client-smoke coverage, Tier 1 e2e config check, public-surface guard. | Optional: a `.codex-plugin` marketplace listing and a stdio-bridge fallback if upstream streamable-HTTP proves flaky. No runtime to port (MCP host, remote HTTP). |
