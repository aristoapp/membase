...
Use `manifests/claude/mcp.json` as the canonical local MCP example. The same
`mcpServers.membase` entry can be used in a project-scoped `.mcp.json`, a
user/local Claude MCP config, or a future plugin-root `.mcp.json`.
...
For a project-scoped review install, copy the file into the target project's
`.mcp.json` and keep the API key as an environment reference:
...
If the Claude plugin later carries plugin-local MCP config, keep `.mcp.json` at
the plugin root. Do not put commands, skills, hooks, or MCP config inside
`.claude-plugin/`; that directory should contain `plugin.json`.
...
Host-level Claude MCP connection checks remain pending until the shared
