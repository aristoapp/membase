...
adapter-declared local commands without publishing, installing the Hermes
package, or calling the Membase API.

Host-level Hermes MCP connection checks remain pending until the shared
`@membase/mcp-server` package path is available and the final Hermes native
runtime package decision is made.
